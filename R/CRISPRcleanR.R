...
ccr.NormfoldChanges<-function(filename,
    ...
    #sb43 is element check not required again...
    numd<-counts[is.element(counts$sgRNA,rownames(libraryAnnotation)),3:ncol(counts)]
...
ccr.logFCs2chromPos<-function(foldchanges,libraryAnnotation){
    ...
    #sb43 unused variable
    position<-round(startp+(endp-startp)/2)

    #sb43 why rowMean for > 3 replicates how is data analysed for 3 and less replicates
    ...
    # sb43 missing round is that intentional ???? c.f position calculaton
    BP<-round(converted$startp+(converted$endp-converted$startp)/2)
...
ccr.cleanChrm<-function(gwSortedFCs,
    ...
    nsegments<- nrow(my.segment.smoothed.CNA.object$output)
    ...
    for (i in 1:nsegments){
        idxs<-my.segment.smoothed.CNA.object$segRows[i,1]:my.segment.smoothed.CNA.object$segRows[i,2] # 1 - 94 rows
        includedGenes<-unique(gwSortedFCs[idxs,'genes']) # 19 genes
        includedGuides<-ID[range(idxs)] # 94 guide RNA
        ...
        if(nGeneInSeg[i]>=min.ngenes){
            ...
            orSign<-sign(newFC[idxs]) # sb43 never used ???
    ...
    regions<-my.segment.smoothed.CNA.object #sb43 why assigned to same variable  could be done as my.segment.smoothed.CNA.object$output
...
ccr.correctCounts<-function(CL,normalised_counts,correctedFCs_and_segments,
    ...
    for (i in 1:nsegments){
        ...
        ntarg<-length(unique(as.character(libraryAnnotation[segment_guides[[i]],'GENES']))) # sb43 store it in segment dataframe

        guides<-segment_guides[[i]]
        if (ntarg>=minTargetedGenes){
# sb43 unsued variable ????
            FC<-correctedFCs_and_segments$corrected_logFCs[guides,'avgFC']
# sb43 takes control sample data
            if(ncontrols==1){
